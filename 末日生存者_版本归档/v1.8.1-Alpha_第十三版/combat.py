...
class CombatSystem:
    ...
    def start_combat(self, player, enemy_data):
        ...
        if self.is_boss_enemy(enemy):
            self.add_combat_log(f"BOSS战开始：{enemy['name']}！")
        ...
        rounds = 0
        escaped = False
        while (player.health > 0 or getattr(self.game, 'is_explore_mode', lambda: False)()) and enemy['health'] > 0 and rounds < 20:
            rounds += 1
    ...
    def create_enemy(self, enemy_data):
        ...
        enemy['cooldowns'] = {}
        enemy['is_boss'] = bool(base.get('is_boss') or enemy.get('id') in self.BOSS_IDS)
        enemy['boss_phase'] = 1
        return enemy

    BOSS_IDS = ('mutant_behemoth', 'raider_commander', 'rust_colossus', 'spore_titan')

    def is_boss_enemy(self, enemy):
        if not enemy:
            return False
        if enemy.get('is_boss'):
            return True
        return enemy.get('id') in self.BOSS_IDS

    def get_boss_phase(self, enemy):
        max_hp = max(1, int(enemy.get('max_health') or 1))
        hp = max(0, int(enemy.get('health', max_hp)))
    ...
    def choose_boss_action(self, enemy, player):
        phase = self.get_boss_phase(enemy)
        enemy['boss_phase'] = phase
        hp_ratio = enemy.get('health', 1) / float(max(1, enemy.get('max_health', 1)))
        enemy_id = enemy.get('id')
        if phase == 1:
            action_type = 'special' if random.random() < 0.35 else 'attack'
            skill = 'heavy_slam' if enemy_id == 'mutant_behemoth' else 'precision_shot'
        elif phase == 2:
            action_type = 'special'
            skill = 'earth_shatter' if enemy_id in ('mutant_behemoth', 'rust_colossus') else 'command_aura'
        else:
            if hp_ratio <= 0.2 and random.random() < 0.45:
                action_type = 'heal'
                skill = 'regenerate'
            else:
                action_type = 'special'
                skill = 'furious_enrage'
        return {'phase': phase, 'type': action_type, 'skill': skill}

    def execute_boss_action(self, player, enemy, action):
        phase = action.get('phase') or self.get_boss_phase(enemy)
        self.add_combat_log(f"{enemy['name']}进入BOSS阶段{phase}！")
        atype = action.get('type', 'attack')
        if atype == 'heal':
            heal = max(8, int(enemy.get('max_health', 1) * 0.08))
            enemy['health'] = min(enemy['max_health'], enemy['health'] + heal)
            self.add_combat_log(f"{enemy['name']}再生了{heal}点生命！")
            return {'type': 'heal', 'amount': heal, 'phase': phase}
        if atype == 'special':
            skill = action.get('skill') or 'heavy_slam'
            names = {
                'earth_shatter': '裂地',
                'heavy_slam': '重击',
                'furious_enrage': '狂暴',
            ...
            self.add_combat_log(f"{enemy['name']}使出{names.get(skill, skill)}！")
            multiplier = 1.2 if phase == 1 else (1.5 if phase == 2 else 2.0)
            atk = self.calculate_enemy_attack(enemy, player)
            dmg = self.apply_damage(player, int(atk['damage'] * multiplier), atk.get('is_critical'))
            self.add_combat_log(f"造成了{dmg}点伤害！")
            if phase >= 3 and skill == 'furious_enrage':
                player.add_debuff({'name': '眩晕', 'type': 'stun', 'duration': 1})
                self.add_combat_log("狂暴冲击令你短暂眩晕！")
            return {'type': 'special', 'damage': dmg, 'phase': phase, 'skill': skill}
        ...
        if attack['is_critical']:
            ...
            self.add_combat_log(f"{enemy['name']}对你造成了{damage}点伤害！")
        return {'type': 'attack', 'damage': damage, 'phase': phase}
    ...
    def enemy_turn(self, player, enemy):
        if self.is_boss_enemy(enemy):
            action = self.choose_boss_action(enemy, player)
            self.execute_boss_action(player, enemy, action)
            return
