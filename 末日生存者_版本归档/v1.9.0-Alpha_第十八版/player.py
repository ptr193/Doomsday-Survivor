...
class Player:
    ...
    def initialize(self, character_data):
        try:
            ...
            self.nickname = (character_data.get('nickname') or '').strip()
    ...
    def load_data(self, save_data):
        try:
            ...
            self.nickname = (save_data.get('nickname') or '').strip()
    ...
    def get_save_data(self):
        return {
            'name': self.name, 'nickname': self.nickname, 'character_class': self.character_class,
    ...
    def _reseal_integrity(self):
        guard = getattr(self.game, 'integrity', None)
        if guard and hasattr(guard, 'touch_player'):
            guard.touch_player(self)
    ...
    def is_explore_mode(self):
        return bool(getattr(self.game, 'is_explore_mode', lambda: False)())

    def modify_health(self, amount):
        if self.is_explore_mode() and amount < 0:
            return self.health
        old = self.health
