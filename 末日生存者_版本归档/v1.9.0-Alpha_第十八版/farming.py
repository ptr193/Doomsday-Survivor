...
class FarmingSystem:
    ...
    def create_empty_farmland(self, location_id, plots=1):
        farmland = {
            ...
            'last_simulated_time': self.game.game_time.isoformat() if getattr(self.game, 'game_time', None) else None
        }
        loc = self.game.world.get_location_by_id(location_id) if hasattr(self.game, 'world') else None
    ...
    def clear_farmland(self, location_id):
        ...
        if not getattr(self.game, 'has_stamina', lambda n: self.game.player.stamina >= n)(20):
    ...
    def load_data(self, save_data):
        ...
        try:
            ...
            for loc_id, farmland_data in farmlands_data.items():
                ...
                last_sim = farmland_data.get('last_simulated_time')
                if isinstance(last_sim, datetime):
                    farmland_data['last_simulated_time'] = last_sim.isoformat()
    ...
    def get_save_data(self):
        ...
        for loc_id, farmland in self.farmlands.items():
            ...
            last_sim = farmland.get('last_simulated_time')
            if isinstance(last_sim, datetime):
                farmlands[loc_id]['last_simulated_time'] = last_sim.isoformat()
        return {'farmlands': farmlands}

    def _parse_sim_time(self, value):
    ...
    def is_near_farmland(self, location_id):
        if not hasattr(self.game, 'world') or not self.game.world:
            return True
        player_loc = getattr(self.game.world, 'current_location_id', None)
        if not player_loc:
            return True
        hops = self.game.world.get_hop_distance(player_loc, location_id)
        return hops is not None and hops <= 1

    def update_crops_growth(self, hours_passed):
        """附近农田按时间推进；离田过远时暂停，返回后再按时间差追赶。"""
        now = getattr(self.game, 'game_time', None)
        for location_id, farmland in self.farmlands.items():
            nearby = self.is_near_farmland(location_id)
            last = self._parse_sim_time(farmland.get('last_simulated_time'))
            if nearby:
                if now and last:
                    delta_hours = (now - last).total_seconds() / 3600.0
                else:
                    delta_hours = hours_passed
                if delta_hours > 0:
                    for plot in farmland['plots']:
                        if plot.get('crop_type') is not None:
                            self.update_single_crop(plot, delta_hours, farmland)
                if now:
                    farmland['last_simulated_time'] = now.isoformat()
            elif now and not last:
                farmland['last_simulated_time'] = now.isoformat()
    ...
    def plant_crop(self, crop_type, location_id, plot_id=None):
        ...
        plot['weeds'] = 0
        if self.game.game_time:
            farmland['last_simulated_time'] = self.game.game_time.isoformat()
    ...
    def water_crops(self, location_id, plot_id=None):
        ...
        if not getattr(self.game, 'has_stamina', lambda n: self.game.player.stamina >= n)(10):
    ...
    def remove_weeds(self, location_id, plot_id):
        ...
        if not getattr(self.game, 'has_stamina', lambda n: self.game.player.stamina >= n)(5):
            return {'success': False, 'message': '体力不足'}
    ...
    def get_farmland_status(self, location_id):
        ...
        status = {
            ...
            'plots_details': [],
            'paused': not self.is_near_farmland(location_id)
        }
        for plot in farmland['plots']:
            plot_info = {
