...
class GameUI:
    ...
    @staticmethod
    def compute_ui_scale(screen_width, screen_height, baseline_w=1280, baseline_h=720):
        width = max(1, int(screen_width or 0))
        height = max(1, int(screen_height or 0))
        scale = min(width / float(baseline_w), height / float(baseline_h))
        return max(0.75, min(2.0, round(scale, 4)))

    @staticmethod
    def scaled_font_size(base_size, scale=1.0):
        try:
            size = int(round(float(base_size) * float(scale)))
        except (TypeError, ValueError):
            size = int(base_size or 10)
        return max(8, size)

    @staticmethod
    def compute_window_geometry(screen_width, screen_height, ratio=0.8):
        win_width = int(max(1, screen_width) * ratio)
        win_height = int(max(1, screen_height) * ratio)
        return f"{win_width}x{win_height}"
    ...
    def setup_window(self):
        ...
        self.root.bind("<KeyPress-g>", self._on_toggle_view_key)
        self.root.bind("<KeyPress-G>", self._on_toggle_view_key)
    ...
    def _on_window_configure(self, event=None):
        ...
        if widget is not None and widget is not self.root:
            return
        try:
            width = int(getattr(event, 'width', 0) or self.root.winfo_width())
            height = int(getattr(event, 'height', 0) or self.root.winfo_height())
        except Exception:
            return
        if width < 200 or height < 200:
            return
        new_scale = self.compute_ui_scale(width, height)
        if abs(new_scale - getattr(self, 'ui_scale', 1.0)) < 0.05:
            return
        self.ui_scale = new_scale
        self.setup_styles()

    def _on_toggle_view_key(self, event=None):
        widget = getattr(event, 'widget', None) if event else None
        if widget and widget.__class__.__name__ in ("Entry", "Text", "ScrolledText"):
            return
        if getattr(self.game, 'game_state', '') == "playing":
            self.game.toggle_view_mode()
    
    def setup_styles(self):
        style = ttk.Style()
        scale = getattr(self, 'ui_scale', 1.0)
        title = self.scaled_font_size(16, scale)
        subtitle = self.scaled_font_size(12, scale)
        normal = self.scaled_font_size(10, scale)
        status = self.scaled_font_size(9, scale)
        pad_x = max(6, int(10 * scale))
        pad_y = max(3, int(5 * scale))
        style.configure("Title.TLabel", font=("Arial", title, "bold"), foreground="#2C3E50")
        style.configure("Subtitle.TLabel", font=("Arial", subtitle, "bold"), foreground="#34495E")
        style.configure("Normal.TLabel", font=("Arial", normal), foreground="#2C3E50")
    ...
    def create_main_menu(self):
        ...
        t = getattr(getattr(self.game, 'i18n', None), 't', lambda k: k)
        ...
        start_button = ttk.Button(button_frame, text=t('menu.start'), command=self.show_save_slots, style="Action.TButton", width=20)
        start_button.pack(pady=10)
        background_button = ttk.Button(button_frame, text=t('menu.background'), command=self.show_game_background, style="Action.TButton", width=20)
        background_button.pack(pady=10)
        codex_button = ttk.Button(button_frame, text=t('menu.codex'), command=self.show_codex, style="Action.TButton", width=20)
        codex_button.pack(pady=10)
        story_button = ttk.Button(button_frame, text=t('menu.story'), command=self.show_story_book, style="Action.TButton", width=20)
        story_button.pack(pady=10)
        gameplay_button = ttk.Button(button_frame, text=t('menu.gameplay'), command=self.show_gameplay_help, style="Action.TButton", width=20)
        gameplay_button.pack(pady=10)
        mod_button = ttk.Button(button_frame, text=t('menu.mods'), command=self.show_mod_manager, style="Action.TButton", width=20)
        mod_button.pack(pady=10)
        exit_button = ttk.Button(button_frame, text=t('menu.exit'), command=self.game.on_closing, style="Danger.TButton", width=20)
        exit_button.pack(pady=20)
    
    def _mode_label(self, game_mode):
        return "探索" if game_mode == "explore" else "生存"
    ...
    def show_save_slots(self):
        ...
        for i in range(1, 6):
            ...
            try:
                if self.game.save_system.save_exists(i):
                    info = self.game.save_system.get_save_info(i)
                    if not info:
                        raise ValueError("empty save info")
                    save_name = info.get('save_name') or info.get('player_name', '未知')
                    player_name = info.get('player_name', '未知')
                    day_count = info.get('day_count', 1)
                    save_time = info.get('save_time', '未知时间')
                    mode_text = self._mode_label(info.get('game_mode', 'survival'))
                    slot_text = f"存档 {i}: {save_name} [{mode_text}] - {player_name} - 第{day_count}天 - {save_time}"
                    ...
                    rename_button = ttk.Button(slot_frame, text="重命名", command=lambda slot=i: self.rename_save(slot))
                ...
                logging.error(f"读取存档槽失败: {e}")
                error_label = ttk.Label(slot_frame, text=f"存档 {i}: 损坏", style="Normal.TLabel")
                error_label.pack(side="left", padx=10)

    def rename_save(self, save_slot):
        from tkinter import simpledialog
        info = self.game.save_system.get_save_info(save_slot)
        current = (info or {}).get('save_name', '')
        name = simpledialog.askstring("存档命名", "输入存档名称:", initialvalue=current)
        if name is None:
            return
        if self.game.save_system.set_save_name(save_slot, name):
    ...
    def delete_save_confirm(self, save_slot):
        ...
        if result:
            try:
                self.game.save_system.delete_save(save_slot)
            except Exception as e:
    ...
    def show_character_selection(self, save_slot):
        ...
        save_name_frame = ttk.Frame(scrollable)
        save_name_frame.pack(fill="x", padx=50, pady=6)
        ttk.Label(save_name_frame, text="存档名称:", style="Normal.TLabel").pack(side="left")
        save_name_var = tk.StringVar(value="")
        save_name_entry = ttk.Entry(save_name_frame, textvariable=save_name_var, width=24)
        save_name_entry.pack(side="left", padx=8)
        ttk.Label(save_name_frame, text="(可空，默认用角色名)", style="Status.TLabel").pack(side="left")
        nick_frame = ttk.Frame(scrollable)
        nick_frame.pack(fill="x", padx=50, pady=6)
        ttk.Label(nick_frame, text="玩家昵称:", style="Normal.TLabel").pack(side="left")
        nick_var = tk.StringVar(value="")
        nick_entry = ttk.Entry(nick_frame, textvariable=nick_var, width=24)
        nick_entry.pack(side="left", padx=8)
        ttk.Label(nick_frame, text="(可空，界面显示用角色名)", style="Status.TLabel").pack(side="left")
        mode_frame = ttk.Frame(scrollable)
        mode_frame.pack(fill="x", padx=50, pady=6)
        ttk.Label(mode_frame, text="游戏模式:", style="Normal.TLabel").pack(side="left")
        mode_var = tk.StringVar(value="survival")
        ttk.Radiobutton(mode_frame, text="生存模式", variable=mode_var, value="survival").pack(side="left", padx=8)
        ttk.Radiobutton(mode_frame, text="探索模式", variable=mode_var, value="explore").pack(side="left", padx=8)
        ttk.Label(scrollable, text="探索模式：资源不会耗尽，不会死亡，行动不消耗体力与生存条。", style="Status.TLabel").pack(anchor="w", padx=50, pady=4)
        def start_with_character(char, slot):
            data = dict(char)
            custom_name = name_var.get().strip()
            data["name"] = custom_name or char["name"]
            data["nickname"] = nick_var.get().strip()
            self.game.start_new_game(slot, data, save_name=save_name_var.get().strip(), game_mode=mode_var.get())
    ...
    def create_game_interface(self):
        ...
        self.create_graphic_map_frame(left_frame)
        # 左侧下层：游戏日志（带滚动）
    ...
    def create_status_bar(self):
        ...
        self.view_toggle_button = ttk.Button(frame, text="图形模式(G)", command=self.game.toggle_view_mode)
        self.view_toggle_button.pack(side="right", padx=5)
    ...
    def create_surroundings_frame(self, parent):
        ...
        self.surroundings_container = label_frame
    ...
    def update_surroundings(self):
        ...
        if getattr(self.game, 'is_graphic_view', lambda: False)():
            self.update_graphic_map()
    ...
    def create_log_frame(self, parent):
        ...
        label_frame.pack(fill="both", expand=True, pady=5)
        self.log_container = label_frame
        self.log_text = scrolledtext.ScrolledText(label_frame, wrap=tk.WORD, font=("Arial", 9))
        self.log_text.pack(fill="both", expand=True)
        self.log_text.config(state="disabled")

    def create_graphic_map_frame(self, parent):
        self.graphic_frame = ttk.LabelFrame(parent, text="俯视地图 (G切换文字/图形)", padding=5)
        self.map_canvas = tk.Canvas(self.graphic_frame, bg="#1a1a1a", height=260, highlightthickness=0)
        self.map_canvas.pack(fill="both", expand=True)
        self.map_canvas.bind("<Button-1>", self._on_graphic_map_click)
        hint = ttk.Label(self.graphic_frame, text="点击地点移动  |  黄点=NPC  |  暗点=未发现  |  G 切换", style="Status.TLabel")
        hint.pack(anchor="w", pady=(4, 0))

    def apply_view_mode(self):
        graphic = self.game.is_graphic_view() if hasattr(self.game, 'is_graphic_view') else False
        log_widget = getattr(self, 'log_container', None)
        surroundings = getattr(self, 'surroundings_container', None)
        graphic_frame = getattr(self, 'graphic_frame', None)
        pack_kw = {'fill': 'both', 'expand': True, 'pady': 5}
        if log_widget:
            pack_kw['before'] = log_widget
        if graphic:
            if surroundings:
                surroundings.pack_forget()
            if graphic_frame:
                graphic_frame.pack(**pack_kw)
                self.update_graphic_map()
        else:
            if graphic_frame:
                graphic_frame.pack_forget()
            if surroundings:
                surroundings.pack(**pack_kw)
        if getattr(self, 'view_toggle_button', None):
            label = "文字模式(G)" if graphic else "图形模式(G)"
            try:
                self.view_toggle_button.config(text=label)
            except Exception:
                pass

    def update_graphic_map(self):
        canvas = getattr(self, 'map_canvas', None)
        if not canvas or not hasattr(self.game, 'world'):
            return
        try:
            canvas.delete("all")
        except Exception:
            pass
        snapshot = self.game.world.get_map_snapshot() if hasattr(self.game.world, 'get_map_snapshot') else {'nodes': [], 'edges': []}
        nodes = snapshot.get('nodes') or []
        if not nodes:
            canvas.create_text(120, 80, text="暂无已发现地点", fill="#cccccc")
            return
        style = snapshot.get('style') or {}
        legend = snapshot.get('legend') or {}
        xs = [n.get('x', 0) or 0 for n in nodes]
        ys = [n.get('y', 0) or 0 for n in nodes]
        ...
        scale = min((max(width - pad * 2 - legend_w, 80)) / span_x, (max(height - pad * 2, 80)) / span_y)
        bg = style.get('background', '#1a1a1a')
        try:
            canvas.config(bg=bg)
        except Exception:
            pass
        if style.get('grid'):
            grid_color = style.get('grid_color', '#2a2a2a')
            for gx in range(0, int(width), 28):
                canvas.create_line(gx, 0, gx, height, fill=grid_color)
            for gy in range(0, int(height), 28):
                canvas.create_line(0, gy, width, gy, fill=grid_color)

        def to_canvas(x, y):
            return pad + (x - min_x) * scale, pad + (y - min_y) * scale

        pos = {n['id']: to_canvas(n.get('x', 0) or 0, n.get('y', 0) or 0) for n in nodes}
        fog_color = style.get('fog_color', '#1c1c1c')
        npc_color = style.get('npc_marker', '#f4d35e')
        current_outline = style.get('current_outline', '#ffcc66')
        for edge in snapshot.get('edges') or []:
            a, b = pos.get(edge.get('from')), pos.get(edge.get('to'))
            if a and b:
                canvas.create_line(a[0], a[1], b[0], b[1], fill="#555555", width=2)
        for n in nodes:
            x, y = pos[n['id']]
            discovered = n.get('discovered', True)
            color = n.get('terrain_color') or '#cccccc'
            if n.get('fog') or not discovered:
                color = fog_color
            size = 14 if n.get('current') else 10
            outline = current_outline if n.get('current') else (n.get('safety_color') or 'white' if discovered else '#444444')
            canvas.create_oval(x - size, y - size, x + size, y + size, fill=color, outline=outline, width=2, tags=n['id'])
            label = n.get('name', n['id']) if discovered else "?"
            canvas.create_text(x, y - 18, text=label, fill="white" if discovered else "#666666", font=("Arial", 8), tags=n['id'])
            if discovered and n.get('npc_ids'):
                canvas.create_oval(x + 6, y - 6, x + 12, y, fill=npc_color, outline="", tags=n['id'])
        ly = 8
        canvas.create_text(width - 46, ly, text="图例", fill="#dddddd", font=("Arial", 8))
        ly += 14
        for tid, info in list(legend.items())[:6]:
            c = info.get('color', '#cccccc') if isinstance(info, dict) else '#cccccc'
            name = info.get('name', tid) if isinstance(info, dict) else tid
            canvas.create_rectangle(width - 88, ly - 5, width - 76, ly + 5, fill=c, outline="#888")
            canvas.create_text(width - 44, ly, text=name, fill="#cccccc", font=("Arial", 8))
            ly += 14

    def _on_graphic_map_click(self, event):
        canvas = getattr(self, 'map_canvas', None)
        if not canvas:
            return
        items = canvas.find_withtag("current") if hasattr(canvas, 'find_withtag') else []
        if not items:
            return
        tags = canvas.gettags(items[0]) if hasattr(canvas, 'gettags') else ()
        if not tags:
            return
        loc_id = tags[0]
        locations = getattr(self.game.world, 'locations', {})
    ...
    def do_surrounding_action(self, obj, service):
        ...
        elif service in ("healing", "radiation_treatment", "weapon_training", "training",
                         "meditation", "information", "farming_tips", "security_tips",
                         "map_info", "location_tips", "survival_tips", "weapon_info",
                         "cooking", "broadcast", "fishing_tips"):
            result = self.game.npcs.provide_service(npc_id, service)
            if result.get('need_ui') and service in ("farming_tips", "security_tips", "map_info",
                                                     "location_tips", "information", "weapon_info", "survival_tips",
                                                     "fishing_tips", "broadcast"):
                self.show_npc_tip(obj, service)
            else:
                message = result.get('message', '没有回应。')
                self.game.add_game_log(message)
                if not result.get('success'):
                    messagebox.showinfo(obj.get('name', 'NPC'), message)
            if result.get('success') and result.get('time_cost'):
                self.game.advance_time(result['time_cost'])
    ...
    def handle_npc_healing(self, npc, service):
        result = self.game.npcs.provide_service(npc.get('id'), service)
        message = result.get('message', '治疗失败')
        if result.get('success'):
            self.game.add_game_log(message)
            if result.get('time_cost'):
                self.game.advance_time(result['time_cost'])
        else:
            messagebox.showwarning("治疗", message)
    ...
    def update_status_display(self):
        ...
        mode_tag = "探索" if getattr(self.game, 'is_explore_mode', lambda: False)() else "生存"
        display_name = p.get_display_name() if hasattr(p, 'get_display_name') else p.name
        if getattr(self.game, 'is_explore_mode', lambda: False)():
            status_text = (f"{display_name} [{mode_tag}] | 生命: -- | 体力: -- | 精神: -- | 疲劳: -- | "
                           f"第{self.game.day_count}天 {self.game.format_time()} {time_name} | "
                           f"{self.game.get_weather_name()} | {self.game.get_season_name()} | 负重 {weight}/{max_weight}{overload}")
            summary_text = f"探索模式  金钱{p.money}  负重{weight}/{max_weight}"
        else:
            status_text = (f"{display_name} [{mode_tag}] | 生命: {p.health}/{p.max_health} | 体力: {p.stamina}/{p.max_stamina} | "
                           ...
                           f"{self.game.get_weather_name()} | {self.game.get_season_name()} | 负重 {weight}/{max_weight}{overload}")
            summary_text = f"生命{p.health}/{p.max_health}  体力{p.stamina}/{p.max_stamina}  精神{p.mental}/{p.max_mental}  金钱{p.money}  负重{weight}/{max_weight}"
    ...
    def update_quest_display(self):
        ...
        if not active_quests:
            ...
            for quest_id in active_quests[:3]:
                ...
                if progress:
                    qname = progress['quest']['name']
                    if self.game.quests.is_epic_quest(quest_id):
                        qname = f"[史诗] {qname}"
                    quest_text += f"● {qname}\n"
    ...
    def show_farming(self):
        ...
        if status.get('paused'):
            ttk.Label(win, text="你离这里较远，作物生长已暂停。返回附近后会按离开时长补算。").pack(pady=6)
        notebook = ttk.Notebook(win)
        notebook.pack(fill="both", expand=True, padx=10, pady=10)
    ...
    def create_active_quests_tab(self, parent):
        ...
        for quest_id in active_quests:
            ...
            if progress:
                ...
                prefix = "[史诗] " if quest.get('category') == 'epic' else ""
                text_widget.insert(tk.END, f"【{prefix}{quest['name']}】\n")
    ...
    def create_available_quests_tab(self, parent):
        ...
        for quest in available:
            ...
            frame.pack(fill="x", padx=10, pady=5)
            qlabel = quest['name']
            if quest.get('category') == 'epic':
                qlabel = f"[史诗] {qlabel}"
            ttk.Label(frame, text=qlabel, style="Subtitle.TLabel").pack(anchor="w")
    ...
    def change_nickname(self):
        from tkinter import simpledialog
        current = getattr(self.game.player, 'nickname', '') or ''
        name = simpledialog.askstring("修改昵称", "输入玩家昵称（可空，显示角色名）:", initialvalue=current)
        if name is None:
            return
        self.game.player.set_nickname(name)
        display = self.game.player.get_display_name()
        self.game.add_game_log(f"你的废土称呼变为：{display}")
    ...
    def show_settings_dialog(self):
        ...
        dialog.grab_set()
        t = getattr(getattr(self.game, 'i18n', None), 't', lambda k: k)
        ttk.Label(dialog, text=t('menu.settings'), style="Title.TLabel").pack(pady=10)
        lang_frame = ttk.Frame(dialog)
        lang_frame.pack(fill="x", padx=20, pady=10)
        ttk.Label(lang_frame, text=t('settings.language') + ":").pack(side="left")
        lang_var = tk.StringVar(master=dialog, value=getattr(getattr(self.game, 'i18n', None), 'language', 'zh'))
        for code, label_key in (('zh', 'lang.zh'), ('en', 'lang.en'), ('ru', 'lang.ru')):
            ttk.Radiobutton(lang_frame, text=t(label_key), variable=lang_var, value=code).pack(side="left", padx=4)
        ...
        def apply_settings():
            ...
            if hasattr(self.game, 'i18n') and self.game.i18n:
                self.game.i18n.set_language(lang_var.get())
            dialog.destroy()
        ttk.Button(dialog, text=t('settings.apply'), command=apply_settings).pack(pady=10)
        ttk.Button(dialog, text=t('settings.cancel'), command=dialog.destroy).pack(pady=5)
    ...
    def _show_story_content(self, story):
        ...
        if story_id and hasattr(self.game, 'quests'):
            self.game.quests.update_quest_progress('document_read', item_id=story_id)
...
- 探索模式：开局可选，资源不会耗尽、不会死亡、不显示消耗条，用于熟悉玩法
...
祝你好运，幸存者！
    ...
    def show_dialogue_window(self, npc, initial_dialogue):
        ...
        entry = ttk.Entry(chat_frame)
        entry.pack(side="left", fill="x", expand=True)

        def send_chat(_event=None):
            spoken = entry.get().strip()
            if not spoken:
                return
            reply = self.game.npcs.chat(npc.get('id'), spoken)
            dialogue_text.config(state="normal")
            dialogue_text.insert(tk.END, f"\n\n你: {spoken}\n{npc.get('name', 'NPC')}: {reply}")
            dialogue_text.see(tk.END)
            dialogue_text.config(state="disabled")
            entry.delete(0, "end")
            self.game.npcs.record_interaction(npc.get('id'))
            if hasattr(self.game, 'player') and hasattr(self.game.player, 'gain_skill_exp'):
                self.game.player.gain_skill_exp('social', 5)

        ttk.Button(chat_frame, text="说", command=send_chat).pack(side="right", padx=5)
    ...
    def show_full_status(self):
        ...
        text.pack(fill="both", expand=True, padx=10, pady=10)
        mode_name = "探索模式" if getattr(self.game, 'is_explore_mode', lambda: False)() else "生存模式"
        display_name = p.get_display_name() if hasattr(p, 'get_display_name') else p.name
        text.insert(tk.END, f"角色: {p.name}\n")
        if getattr(p, 'nickname', ''):
            text.insert(tk.END, f"昵称: {p.nickname}\n")
        text.insert(tk.END, f"显示名: {display_name}\n")
        text.insert(tk.END, f"模式: {mode_name}\n")
        if getattr(self.game, 'is_explore_mode', lambda: False)():
            text.insert(tk.END, "生命/体力/精神/疲劳: 探索模式不显示消耗条\n")
        else:
