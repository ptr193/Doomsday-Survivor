...
class TextAdventureGame:
    ...
    def initialize_systems(self):
        ...
        try:
            ...
            from world import GameWorld, TerrainGenerator
            ...
            from save_system import SaveSystem, DiarySystem
            from achievement import AchievementSystem
            from mod_manager import ModManager
            from story_reader import StoryReader
            from utils import IntegrityGuard, I18n
    ...
    def is_explore_mode(self):
        return getattr(self, 'game_mode', 'survival') == 'explore'

    def is_graphic_view(self):
        return getattr(self, 'view_mode', 'text') == 'graphic'

    def toggle_view_mode(self):
        self.view_mode = 'text' if self.is_graphic_view() else 'graphic'
        if hasattr(self, 'ui') and self.ui:
            self.ui.apply_view_mode()
        mode_name = "图形模式" if self.is_graphic_view() else "文字模式"
        self.add_game_log(f"已切换到{mode_name}。输入 G 可再次切换。")

    def has_stamina(self, amount):
        if self.is_explore_mode():
            return True
        return self.player.stamina >= amount

    def start_new_game(self, save_slot, character_data, save_name="", game_mode="survival"):
        """开始新游戏"""
        try:
            logging.info(f"开始新游戏，存档槽: {save_slot}")
            self.current_save_slot = save_slot
            self.game_mode = game_mode if game_mode in ("survival", "explore") else "survival"
            player_name = character_data.get("name", "幸存者")
            self.save_name = (save_name or "").strip() or player_name
            ...
            self.fatigue = 0
            self.completed_research = []
            self.view_mode = getattr(self, 'view_mode', 'text') or 'text'
            ...
            if self.is_explore_mode():
                self.add_game_log("探索模式已开启：资源不会耗尽，不会死亡，行动不消耗体力与生存条。")
            self.save_game()
    ...
    def load_game(self, save_slot):
        ...
        try:
            ...
            if not save_data:
                ...
                return
            self.current_save_slot = save_slot
            self.game_mode = save_data.get('game_mode') or (save_data.get('metadata') or {}).get('game_mode') or 'survival'
            self.view_mode = save_data.get('view_mode') or (save_data.get('metadata') or {}).get('view_mode') or 'text'
            self.save_name = save_data.get('save_name') or (save_data.get('metadata') or {}).get('save_name') or save_data.get('player', {}).get('name', '')
            ...
            display = self.player.get_display_name() if hasattr(self.player, 'get_display_name') else self.player.name
            self.add_game_log(f"游戏加载成功！欢迎回来，{display}。")
            self.add_game_log(f"现在是第{self.day_count}天，{self.format_time()}。")
            if self.is_explore_mode():
                self.add_game_log("当前为探索模式。")
            logging.info("游戏加载成功")
        except Exception as e:
    ...
    def save_game(self):
        ...
        try:
            save_data = {
                ...
                'save_name': getattr(self, 'save_name', '') or self.player.name,
                'game_mode': getattr(self, 'game_mode', 'survival'),
                'view_mode': getattr(self, 'view_mode', 'text')
            }
    ...
    def trigger_initial_events(self):
        display = self.player.get_display_name() if hasattr(self.player, 'get_display_name') else self.player.name
        self.add_game_log(f"欢迎，{display}！你开始了在末日世界的生存之旅。")
    ...
    def perform_action(self, action_type, **kwargs):
        ...
        try:
            ...
            elif action_type == "npc_service":
                self.action_npc_service(kwargs.get('npc_id'), kwargs.get('service'), **kwargs)
            else:
    ...
    def action_move(self, location_id):
        if not location_id:
            ...
            return
        if self.player.is_overencumbered() and not self.has_stamina(12):
    ...
    def action_hunt(self):
        ...
        if random.random() < success_chance * weather_mod:
            pool = self.world.get_wildlife_pool(loc) if hasattr(self.world, 'get_wildlife_pool') else []
            creature = random.choice(pool) if pool else {'name': '野兽', 'loot': {'food': [2, 5], 'leather': [0, 2]}}
            parts = [f"猎到了{creature.get('name', '野兽')}"]
            loot = creature.get('loot', {'food': [2, 5]})
            for item_id, amount in loot.items():
                if isinstance(amount, (list, tuple)):
                    low, high = int(amount[0]), int(amount[-1])
                    qty = random.randint(low, max(low, high))
                else:
                    qty = int(amount)
                if qty > 0:
                    self.player.add_item(item_id, qty)
                    parts.append(f"{self.items.get_item_name(item_id)}x{qty}")
            self.add_game_log("狩猎成功！" + "，".join(parts))
    ...
    def action_npc_service(self, npc_id, service, **kwargs):
        if not npc_id or not service:
            self.add_game_log("请指定要交互的NPC和服务。")
            return
        result = self.npcs.provide_service(npc_id, service, **kwargs)
        self.add_game_log(result.get('message', '没有回应。'))
        if result.get('success') and result.get('time_cost'):
            self.advance_time(result['time_cost'])
    ...
    def execute_command(self, cmd):
        ...
        cmd = cmd.strip().lower()
        if cmd == "help":
            self.add_game_log("可用指令: help, status, inventory, map, g, sleep, eat, drink, explore, quit")
        elif cmd == "status":
