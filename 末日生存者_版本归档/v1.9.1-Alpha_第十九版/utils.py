...
class IntegrityGuard:
    WATCH_FIELDS = (
        'health', 'max_health', 'stamina', 'max_stamina', 'mental',
        'money', 'radiation', 'strength', 'agility', 'intelligence',
        'endurance', 'luck'
    )
    SECRET = b'doomsday-survivor-1.8.4-local'
    ...
    def _digest(self, payload):
        raw = self._canonical(payload).encode('utf-8')
        return hmac.new(self.SECRET, raw, hashlib.sha256).hexdigest()

    def _player_snapshot(self, player):
        snap = {}
        for field in self.WATCH_FIELDS:
            snap[field] = getattr(player, field, None)
        return snap

    def seal_player(self, player):
        self._player_token = self._digest(self._player_snapshot(player))
        return self._player_token

    def touch_player(self, player):
        return self.seal_player(player)

    def verify_player(self, player):
        if not self._player_token:
            return False
        expected = self._digest(self._player_snapshot(player))
        return hmac.compare_digest(self._player_token, expected)

    def sign_save(self, save_data):
        payload = {k: v for k, v in dict(save_data).items() if k != 'integrity'}
        token = self._digest(payload)
        signed = dict(save_data)
        signed['integrity'] = token
        return signed

    def verify_save(self, save_data):
        if not isinstance(save_data, dict) or 'integrity' not in save_data:
            return False
        token = save_data.get('integrity')
        payload = {k: v for k, v in save_data.items() if k != 'integrity'}
        return hmac.compare_digest(str(token), self._digest(payload))


class PlatformPaths:
    APP_ID = 'doomsday-survivor'

    def __init__(self, platform=None, home=None, appdata=None):
        self.platform = (platform if platform is not None else sys.platform) or 'linux'
    ...
    def user_data_dir(self):
        ...
        if plat.startswith('win'):
            base = self.appdata or os.path.join(self.home, 'AppData', 'Roaming')
            return os.path.join(base, 'DoomsdaySurvivor')
        if plat == 'darwin':
            return os.path.join(self.home, 'Library', 'Application Support', 'DoomsdaySurvivor')
        if plat == 'android':
            return os.path.join(self.home, 'files', 'doomsday-survivor')
        return os.path.join(self.home, '.local', 'share', self.APP_ID)

    @property
    def saves_dir(self):
        return os.path.join(self.user_data_dir, 'saves')

    @property
    def logs_dir(self):
        return os.path.join(self.user_data_dir, 'logs')
...
class I18n:
    ...
    STRINGS = {
        ...
        'ru': {
            ...
            'menu.exit': 'Выход',
            'menu.background': 'Предыстория',
            'menu.codex': 'Кодекс',
            'menu.story': 'Книга историй',
            'menu.gameplay': 'Как играть',
            'menu.mods': 'Моды',
    ...
    def _normalize(self, language):
        raw = str(language or self.FALLBACK).replace('_', '-').lower()
    ...
    def t(self, key, **kwargs):
        table = self.STRINGS.get(self.language) or self.STRINGS[self.FALLBACK]
        text = table.get(key)
        if text is None:
            text = self.STRINGS[self.FALLBACK].get(key, key)
        if kwargs:
            try:
