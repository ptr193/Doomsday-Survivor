...
class SaveSystem:
    def __init__(self, game):
        ...
        try:
            ...
            paths = PlatformPaths()
            self.save_dir = paths.saves_dir
        except Exception:
            self.save_dir = os.path.abspath("saves")
        self.backup_dir = os.path.join(self.save_dir, "backups")
    ...
    def get_slot_dir(self, save_slot: int) -> str:
        return os.path.join(self.save_dir, f"save_slot_{save_slot}")

    def get_folder_save_path(self, save_slot: int) -> str:
        return os.path.join(self.get_slot_dir(save_slot), "game.json")

    def get_legacy_save_path(self, save_slot: int) -> str:
        return os.path.join(self.save_dir, f"save_slot_{save_slot}.json")

    def get_save_file_path(self, save_slot: int) -> str:
        folder_file = self.get_folder_save_path(save_slot)
        if os.path.isfile(folder_file):
            return folder_file
        legacy = self.get_legacy_save_path(save_slot)
        if os.path.isfile(legacy):
            return legacy
        return folder_file

    def save_exists(self, save_slot: int) -> bool:
        return os.path.isfile(self.get_folder_save_path(save_slot)) or os.path.isfile(self.get_legacy_save_path(save_slot))

    def save_game(self, save_slot: int, save_data: Dict[str, Any]) -> bool:
        try:
            slot_dir = self.get_slot_dir(save_slot)
            os.makedirs(slot_dir, exist_ok=True)
            save_file = self.get_folder_save_path(save_slot)
            if os.path.exists(save_file) or os.path.isfile(self.get_legacy_save_path(save_slot)):
                self.create_backup(save_slot)
            player_name = save_data.get('player', {}).get('name', '未知')
            save_name = save_data.get('save_name') or (save_data.get('metadata') or {}).get('save_name') or player_name
            save_data['save_name'] = save_name
            save_data['game_mode'] = save_data.get('game_mode', getattr(self.game, 'game_mode', 'survival'))
            save_data['view_mode'] = save_data.get('view_mode', getattr(self.game, 'view_mode', 'text'))
            save_data['metadata'] = {
                'version': getattr(self.game, 'version', save_data.get('version', 'unknown')),
                'save_slot': save_slot,
                'save_time': datetime.now().isoformat(),
                'player_name': player_name,
                'day_count': save_data.get('day_count', 1),
                'save_name': save_name,
                'game_mode': save_data.get('game_mode', 'survival'),
                'view_mode': save_data.get('view_mode', 'text')
            }
            guard = getattr(self.game, 'integrity', None)
            if guard and hasattr(guard, 'sign_save'):
                save_data = guard.sign_save(save_data)
            with open(save_file, 'w', encoding='utf-8') as f:
                json.dump(save_data, f, ensure_ascii=False, indent=2)
            logging.info(f"游戏保存成功: 槽位 {save_slot} -> {save_file}")
            return True
    ...
    def load_game(self, save_slot: int) -> Optional[Dict[str, Any]]:
        try:
            ...
            guard = getattr(self.game, 'integrity', None)
            if guard and hasattr(guard, 'verify_save') and 'integrity' in save_data:
                if not guard.verify_save(save_data):
                    logging.error("存档完整性校验失败")
                    return self.restore_backup(save_slot)
    ...
    def delete_save(self, save_slot: int) -> bool:
        try:
            ...
            if removed:
                ...
                return True
            logging.warning(f"存档不存在: 槽位 {save_slot}")
            return False
        except Exception as e:
            logging.error(f"删除存档失败: {e}")
            return False

    def set_save_name(self, save_slot: int, save_name: str) -> bool:
        save_data = self.load_game(save_slot)
        if not save_data:
            return False
        name = (save_name or "").strip() or save_data.get('player', {}).get('name', '未命名存档')
    ...
    def get_save_info(self, save_slot: int) -> Optional[Dict[str, Any]]:
        try:
            ...
            save_name = save_data.get('save_name') or metadata.get('save_name') or player_name
            game_mode = save_data.get('game_mode') or metadata.get('game_mode') or 'survival'
            return {
                'slot': save_slot,
                'save_name': save_name,
                'game_mode': game_mode,
                'player_name': player_name,
                'day_count': save_data.get('day_count', 1),
                'save_time': metadata.get('save_time', save_data.get('save_time', '未知')),
                'location': player_data.get('location', '未知'),
                'health': player_data.get('health', 0),
                'max_health': player_data.get('max_health', 0),
                'version': metadata.get('version', save_data.get('version', '未知'))
            }
        except Exception as e:
...
class DiarySystem:
    ...
    def initialize(self, save_slot):
        if hasattr(self.game, 'save_system') and self.game.save_system:
            slot_dir = self.game.save_system.get_slot_dir(save_slot)
        else:
            slot_dir = os.path.join("saves", f"save_slot_{save_slot}")
        self.diary_path = os.path.join(slot_dir, "diary.txt")
        self.load()
        self.initialized = True
        logging.info(f"日记系统初始化: {self.diary_path}")

    def load(self):
        if self.diary_path and os.path.exists(self.diary_path):
            try:
