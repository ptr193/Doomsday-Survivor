...
class NPCSystem:
    ...
    def create_npcs(self):
        self.npcs = {
            ...
            'mysterious_stranger': {'id': 'mysterious_stranger', 'name': '神秘陌生人', 'faction': 'neutral', 'location': 'random', 'type': 'special', 'description': '身份不明的旅行者，似乎知道很多秘密', 'dialogue': {'greeting': '你不该来这里...但既然来了，也许能帮上忙。', 'farewell': '小心你信任的人，这个世界已经变了。', 'topics': {'secrets': '有些真相还是不知道为好。', 'prophecy': '星星在移动，平衡正在改变...', 'warning': '不要相信你看到的一切。'}}, 'services': ['information', 'special_quests'], 'quests': [], 'shop': None, 'reputation_required': 50},
    ...
    def buy_item(self, shop_id, item_id, quantity=1):
        ...
        return {'success': True, 'message': f"成功购买{quantity}个{self.game.items.get_item_name(item_id)}", 'item_id': item_id, 'quantity': quantity, 'total_price': total_price}
    ...
    def sell_item(self, shop_id, item_id, quantity=1):
        ...
        return {'success': True, 'message': f"成功出售{quantity}个{self.game.items.get_item_name(item_id)}", 'item_id': item_id, 'quantity': quantity, 'total_price': sell_price}
