...
class GameUI:
    ...
    def create_equipment_tab(self, parent):
    	...
    	for slot, slot_name in slots.items():
    	       ...
    	       # 装备/卸下按钮
    	       if item_id:
    	                 ttk.Button(slot_frame, text="卸下", 
    	                 command=lambda s=slot: self.unequip_item(s)).pack(side="right")
    	       else:
    	                 	ttk.Button(slot_frame, text="装备",
    	                 	command=lambda s=slot: self.show_equip_dialog(s)).pack(side="right")
