...
class GameUI:
    ...
    def create_graphic_map_frame(self, parent):
        ...
        self.map_canvas = tk.Canvas(self.graphic_frame, bg="#1a1a1a", height=240, highlightthickness=0)
        self.map_canvas.pack(fill="both", expand=True)
        self.map_canvas.bind("<Button-1>", self._on_graphic_map_click)
        hint = ttk.Label(self.graphic_frame, text="点击地点移动  |  G 切换模式", style="Status.TLabel")
    ...
    def update_graphic_map(self):
        ...
        scale = min((max(width - pad * 2, 80)) / span_x, (max(height - pad * 2, 80)) / span_y)
        ...
        for edge in snapshot.get('edges') or []:
            ...
            if a and b:
                canvas.create_line(a[0], a[1], b[0], b[1], fill="#666666", width=2)
        colors = {
            "forest": "#2d6a4f", "plain": "#a7c5a3", "mountain": "#8d6b63",
            "river": "#4ea8de", "urban": "#4a6e8c"
        }
        for n in nodes:
            x, y = pos[n['id']]
            discovered = n.get('discovered', True)
            color = colors.get(n.get('terrain'), "#cccccc") if discovered else "#333333"
            size = 14 if n.get('current') else 10
            outline = "#ffcc66" if n.get('current') else ("white" if discovered else "#555555")
            ...
            canvas.create_text(x, y - 18, text=label, fill="white" if discovered else "#777777", font=("Arial", 8), tags=n['id'])
