...
class LocalDialogueEngine:
    TOPIC_ALIASES = {
        'injuries': ('伤', '伤口', '治疗', '疼', '血', '医', '受伤'),
        'medicine': ('药', '药品', '医疗', '治疗', '健康'),
        'radiation': ('辐射', '放射', '核'),
        'farming': ('地', '粮', '农', '种', '田'),
        'survival': ('生存', '活', '安全', '打猎', '猎'),
        'threats': ('晚上', '夜', '安全', '危险', '变异', '怪物'),
        'security': ('安全', '警戒', '防卫', '掠夺'),
        'raiders': ('掠夺', '土匪', '敌人'),
        'weapons': ('武器', '枪', '刀', '装备'),
        'crafting': ('制作', '工艺', '工具'),
        'materials': ('材料', '资源'),
    ...
    def _npc(self, npc_id):
        npcs = getattr(self.game, 'npcs', None)
        if npcs and hasattr(npcs, 'get_npc'):
            return npcs.get_npc(npc_id)
        return None

    def _push(self, npc_id, role, text):
        self._history.setdefault(npc_id, []).append({'role': role, 'text': text})

    def history(self, npc_id):
        return list(self._history.get(npc_id, []))

    def greet(self, npc_id):
        npc = self._npc(npc_id)
        if not npc:
            return "......"
        text = npc.get('dialogue', {}).get('greeting') or "......"
        self._push(npc_id, 'npc', text)
        return text

    def farewell(self, npc_id):
        npc = self._npc(npc_id)
        if not npc:
            return "......"
        text = npc.get('dialogue', {}).get('farewell') or "......"
        self._push(npc_id, 'npc', text)
        return text

    def reply(self, npc_id, player_text):
        npc = self._npc(npc_id)
        if not npc:
            return "......"
        spoken = (player_text or '').strip()
        self._push(npc_id, 'player', spoken)
        base = self._match_topic(npc, spoken)
        flavored = self._apply_context(npc, spoken, base)
        self._push(npc_id, 'npc', flavored)
        return flavored

    def _match_topic(self, npc, spoken):
        dialogue = npc.get('dialogue') or {}
        topics = dict(dialogue.get('topics') or {})
        if dialogue.get('greeting'):
            topics['greeting'] = dialogue['greeting']
        if dialogue.get('farewell'):
            topics['farewell'] = dialogue['farewell']
        if not spoken:
            return dialogue.get('greeting') or "......"
        best_key = None
        best_score = 0
        for key, text in topics.items():
            ...
            if key and key in spoken:
                score += 3
            for alias in self.TOPIC_ALIASES.get(key, ()):
                if alias and alias in spoken:
                    score += 2
            blob = str(text or '')
            for ch in set(spoken):
                if ch.strip() and ch in blob:
                    score += 0.15
            if score > best_score:
                best_score = score
                best_key = key
        if best_key and best_score >= 1.5:
            return topics[best_key]
        return dialogue.get('greeting') or "我现在没什么可说的。"

    def _is_night(self):
        checker = getattr(self.game, 'is_night', None)
        if callable(checker):
            try:
                return bool(checker())
            except Exception:
    ...
    def _apply_context(self, npc, spoken, base):
        extra = ""
        if self._is_night() and any(k in spoken for k in self.NIGHT_HINTS):
            extra = "夜里更危险，务必小心。"
        player = getattr(self.game, 'player', None)
        if player is not None and getattr(player, 'health', 100) < 40:
            if any(k in spoken for k in ('伤', '治疗', '药')):
                extra = extra or "你看起来伤得不轻。"
        if extra and extra not in (base or ""):
            return f"{base}{extra}"
        return base
...
class NPCSystem:
    ...
    def initialize(self):
        try:
            ...
            self.dialogue_engine = LocalDialogueEngine(self.game)
    ...
    def chat(self, npc_id, player_text):
        if not getattr(self, 'dialogue_engine', None):
            self.dialogue_engine = LocalDialogueEngine(self.game)
        return self.dialogue_engine.reply(npc_id, player_text)
    ...
    def provide_service(self, npc_id, service, **kwargs):
        ...
        handler = {
            ...
            'fishing_tips': self._service_tip,
            'cooking': self._service_cooking,
            'broadcast': self._service_broadcast,
        }.get(service)
        if not handler:
        ...
        if result.get('success'):
            ...
            if hasattr(self.game, 'quests') and self.game.quests:
                self.game.quests.update_quest_progress('special_action_done', action=service)
    ...
    def _service_tip(self, npc, service, **kwargs):
        ...
        topic_map = {
            ...
            'survival_tips': 'survival', 'fishing_tips': 'fishing'
        }
    ...
    def _service_cooking(self, npc, service, **kwargs):
        cost = self.get_service_price(npc['id'], 'cooking')
        if cost and not self.game.player.spend_money(cost):
            return {'success': False, 'message': f'金钱不足，热餐需要{cost}'}
        self.game.player.modify_stamina(25)
        self.game.player.modify_health(8)
        msg = f"{npc['name']}给你盛了一碗热食，体力恢复了。"
        if cost:
            msg += f"花费{cost}。"
        return {'success': True, 'message': msg, 'time_cost': 1, 'rep_gain': 1}

    def _service_broadcast(self, npc, service, **kwargs):
        topics = npc.get('dialogue', {}).get('topics', {})
        text = topics.get('broadcast') or topics.get('rumors') or "电台里只有沙沙声。"
        self.game.player.gain_skill_exp('social', 4)
        return {'success': True, 'message': f"{npc['name']}调出一段广播：{text}", 'time_cost': 0.5, 'rep_gain': 1}