...
class NPCSystem:
    ...
    def create_factions(self):
        self.factions = {
            'survivors': {
                'id': 'survivors', 'name': '幸存者联盟', 'description': '致力于重建文明的幸存者组织',
                'alignment': 'good', 'base_reputation': 50,
                'relationships': {'raiders': -100, 'tech_cult': 0, 'traders': 75}
            },
            'raiders': {
                'id': 'raiders', 'name': '掠夺者部落', 'description': '以掠夺和暴力为生的危险组织',
                'alignment': 'evil', 'base_reputation': -50,
                'relationships': {'survivors': -100, 'tech_cult': -50, 'traders': -75}
            },
            'tech_cult': {
                'id': 'tech_cult', 'name': '科技教会', 'description': '崇拜科技的神秘组织',
                'alignment': 'neutral', 'base_reputation': 0,
                'relationships': {'survivors': 0, 'raiders': -50, 'traders': 25}
            },
            'traders': {
                'id': 'traders', 'name': '商人行会', 'description': '专注于贸易的商业组织',
                'alignment': 'neutral', 'base_reputation': 25,
                'relationships': {'survivors': 75, 'raiders': -75, 'tech_cult': 25}
            }
        }
    ...
    def create_npcs(self):
        self.npcs = {
            'old_farmer': {
                'id': 'old_farmer', 'name': '老农民张大爷', 'faction': 'survivors',
                'location': 'abandoned_farm', 'type': 'quest_giver',
            ...
            'doctor_li': {
                'id': 'doctor_li', 'name': '李医生', 'faction': 'survivors',
                'location': 'starting_area', 'type': 'healer',
            ...
            'security_chief': {
                'id': 'security_chief', 'name': '王队长', 'faction': 'survivors',
                'location': 'survivor_camp', 'type': 'guard',
            ...
            'master_crafter': {
                'id': 'master_crafter', 'name': '工匠大师', 'faction': 'survivors',
                'location': 'trading_post', 'type': 'crafter',
            ...
            'cartographer': {
                'id': 'cartographer', 'name': '地图绘制师', 'faction': 'survivors',
                'location': 'survivor_camp', 'type': 'explorer',
            ...
            'wandering_merchant': {
                'id': 'wandering_merchant', 'name': '流浪商人卡尔', 'faction': 'traders',
                'location': 'random', 'type': 'merchant',
            ...
            'weapon_dealer': {
                'id': 'weapon_dealer', 'name': '军火商文森特', 'faction': 'traders',
                'location': 'trading_post', 'type': 'merchant',
            ...
            'mad_scientist': {
                'id': 'mad_scientist', 'name': '疯狂科学家', 'faction': 'tech_cult',
                'location': 'research_lab', 'type': 'researcher',
            ...
            'mysterious_stranger': {
                'id': 'mysterious_stranger', 'name': '神秘陌生人', 'faction': 'neutral',
                'location': 'random', 'type': 'special',
            ...
            'hermit': {
                'id': 'hermit', 'name': '山中隐士', 'faction': 'neutral',
                'location': 'mountain_path', 'type': 'teacher',
        ...
        self.shop_inventories = {
            ...
            'weapon_shop': {
                'items': {
                    ...
                    'bow': {'price': 35, 'stock': 4},
                    'arrow': {'price': 2, 'stock': 50},
                    'cloth_armor': {'price': 25, 'stock': 6},
                    'leather_armor': {'price': 45, 'stock': 4}
                },
                'buys': ['knife', 'baseball_bat', 'pistol', 'cloth_armor', 'arrow'],
                'price_modifier': 1.5
            },
            'crafting_shop': {
                'items': {
                    ...
                    'fishing_rod': {'price': 20, 'stock': 6},
                    'trap': {'price': 25, 'stock': 4}
                },
            ...
            'tech_shop': {
                'items': {
                    ...
                    'battery': {'price': 10, 'stock': 20},
                    'solar_panel': {'price': 100, 'stock': 2}
                },
    ...
    def get_save_data(self):
        ...
        for shop_id, shop in self.shop_inventories.items():
            shops_data[shop_id] = {
                'items': {iid: {'stock': info['stock']} for iid, info in shop['items'].items()}
            }
        return {
    ...
    def get_npcs_at_location(self, location_id):
        ...
        for npc_id, npc in self.npcs.items():
            if npc['location'] == location_id:
                result.append(npc)
            elif npc['location'] == 'random' and random.random() < 0.3:
                result.append(npc)
        return result
    ...
    def get_player_relationships(self):
        ...
        for fid, faction in self.factions.items():
            res[fid] = {
                'faction': faction,
                'reputation': self.relationships.get(fid, faction['base_reputation']),
                'level': self.get_relationship_level(fid)
            }
        return res
